...
The relevant values are then interpolated on to the uniform vector xx.  There are two methods for doing this, only one is active at a time and the other is commented out.  These are:

1. [h,a,k,phi,xxi,eta,xx,yy]=fit\_sine\_curve(filename);
